...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)\
            .solve(train_data, train_labels, val_data, val_labels,model)
        if accuracy < model.accuracy(val_data, val_labels):
            best_model = model
