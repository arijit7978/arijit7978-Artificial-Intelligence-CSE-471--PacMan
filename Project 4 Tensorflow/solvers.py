...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(grad_tensors)):
            new_par_var = param_vars[i] - self.learning_rate * grad_tensors[i]
            updates.append((param_vars[i], new_par_var))
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(vel_vars)):
            ...
            new_par_var = param_vars[i] + new_vel_var
            updates.extend(((vel_vars[i], new_vel_var), (param_vars[i], new_par_var)))
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_loss, W2_value = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: train_data[0], placeholders[1]: train_data[1]})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        batch_train = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        batch_val = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        for iter_ in xrange(self.iterations):
            "*** YOUR CODE HERE ***"
            input_train, target_train = batch_train.next()
            input_val, target_val = batch_val.next()
            train_loss, W2_value = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: input_train, placeholders[1]: target_train})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: input_val, placeholders[1]: target_val})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            input_val, target_val = valGenerator.next()
            train_loss, W2_value = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: input_train, placeholders[1]: target_train})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: input_val, placeholders[1]: target_val})
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            # val_loss should be the loss of this iteration using the same amount of data used for the updates, but using the validation data instead
