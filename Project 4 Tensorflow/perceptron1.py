...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
       ...
       # for iteration in range(1):
           ...
           # for i in range(1):
                ...
                "*** YOUR CODE HERE ***"
                y_pred = max([(self.get_score(input_train_data[i], y), y) for y in self.weights.keys()])[1]
                if y_pred != label_train_data[i]:
                    for y in self.weights.keys():
                        if y == label_train_data[i]:
                            self.weights[y] += input_train_data[i] 
                        else:
                            self.weights[y] -= input_train_data[i] 

    def get_score(self, f, y):
        return f.__mul__(self.weights[y])
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        #Getting the weight vector for a given label
        weightVector = self.weights[label]

        #Getting the keys of the weight vector, sorted from greatest to least
        sortedKeys = weightVector.sortedKeys()

        #Getting the 100 best features of
        best100Features = sortedKeys[:100]
